...
# allowed arguments for GENIA dataset
def valid_argument_name(a):
    return re.match(r"^Cause|CSite|ToLoc|AtLoc|Site|Site2|(Theme)\d*$", a) is not None


# allowed (argument, referred-type) combinations
def valid_argument_type(arg, reftype):
    if arg == "Cause" or re.match(r"^(Theme)\d*$", arg):
        return reftype in ["Protein", "Event"] + list(event_types)
    if arg in ("ToLoc", "AtLoc", "CSite", "Site", "Site2"):
        return reftype in ["Entity"]
...
class Textbound:
    ...
    def verify_text(self, document_text, fn):
        ...
        assert self.start < len(document_text) and self.end <= len(document_text), (
            "FORMAT ERROR: textbound offsets extend over document length: '%s' in %s"
            % (self.to_string(), fn)
        )
        assert (
            document_text[self.start : self.end].strip().replace("\n", " ")
            == self.text.strip()
        ), (
            "FORMAT ERROR: text '%s' referenced by [start, end) doesn't match given text '%s': '%s' in %s"
    ...
    def identify_extended_span(self, document_text, reserved_index):
        ...
        while (
            estart - 1 >= 0
            and estart - 1 not in reserved_index
            and not document_text[estart - 1].isspace()
            and document_text[estart - 1] not in punctuation_chars
        ):
            estart -= 1
        ...
        for i in range(1):
            ...
            if eend > len(document_text) or eend - 1 in reserved_index:
                eend -= 1
        while (
            eend < len(document_text)
            and eend not in reserved_index
            and not document_text[eend].isspace()
            and document_text[eend] not in punctuation_chars
        ):
            eend += 1
    ...
    def matches_self(self, t):
        ...
        if options.softboundary and self.is_trigger:
            ...
            if t.extended_start is not None:
                ...
                return t.extended_start <= self.start and t.extended_end >= self.end
            assert self.extended_start is not None and self.extended_end is not None, (
                "INTERNAL ERROR"
            )
    ...
    def matches(self, t, dummy=False):
        ...
        # docIDとイベントIDの出力
        # print("Comparing text:", getattr(self, 'text', 'Unknown'), "イベントID:", getattr(self, 'id', 'Unknown'))
...
class Equiv:
    ...
    def resolve_idrefs(self, annotation_by_id):
        self.entities = []
        for id in self.ids:
            assert id in annotation_by_id, (
                "ERROR: undefined ID %s referenced in Equiv" % id
            )
            ...
            assert e not in self.entities, (
                "ERROR: %s with ID %s occurs multiple times in Equiv" % (e.type, id)
            )
...
class Event:
    def __init__(self, id, type, tid, args):
        ...
        for arg, aid in self.args:
            assert aid != self.id, (
                "ERROR: event ID %s contains argument %s referring to itself"
                % (self.id, arg)
            )
    ...
    def resolve_idrefs(self, annotation_by_id):
        ...
        assert self.tid in annotation_by_id, (
            "ERROR: undefined trigger ID %s referenced in event %s"
            % (self.tid, self.id)
        )
        self.trigger = annotation_by_id[self.tid]
        assert self.trigger.type == self.type, (
            "ERROR: trigger %s type disagrees with event %s type" % (self.tid, self.id)
        )
        ...
        for arg, aid in self.args:
            if aid not in annotation_by_id:
                assert aid in annotation_by_id, (
                    "ERROR: undefined ID %s referenced in event %s" % (aid, self.id)
                )

            ref = annotation_by_id[aid]
            if not valid_argument_type(arg, ref.type):
                assert valid_argument_type(arg, ref.type), (
                    "ERROR: argument %s for %s event %s has invalid type %s"
            ...
            if m:
                ...
                assert arg not in self.arg, (
                    "ERROR: event %s has multiple %s arguments" % (self.id, arg)
                )
                self.arg[arg] = ref
        ...
        for argnum in theme_site_nums:
            ...
            if not options.spliteventeval and self.type != "Mutation":
                assert t_arg in theme_site_dict, (
                    "ERROR: event %s has Site%s without Theme%s"
            ...
            if s_arg in theme_site_dict:
                assert len(theme_site_dict[s_arg]) == 1, (
                    "ERROR: event %s has multiple Site%s arguments, Site-Theme pairing is ambiguous"
                    % (self.id, argnum)
                )
                # ("not in" for spliteventeval)
                assert (
                    t_arg not in theme_site_dict or len(theme_site_dict[t_arg]) == 1
                ), (
                    "ERROR: event %s has Site%s and multiple Theme%s arguments, Site-Theme pairing is ambiguous"
                    % (self.id, argnum, argnum)
                )
            ...
            if t_arg in theme_site_dict:
                ...
                assert options.spliteventeval or self.type == "Mutation", (
                    "INTERNAL ERROR"
                )
    ...
    def matches(self, e, match_theme_only=False, match_partial=False):
        ...
        for sts in self_theme_sites:
            for ots in other_theme_sites:
                ...
                if (sts[1] is None and ots[1] is None) or (
                    sts[1] is not None
                    and sts[1].matches(ots[1], options.partialrecursive)
                ):
                    # theme match
                    if (sts[2] is None and ots[2] is None) or (
                        sts[2] is not None
                        and sts[2].matches(ots[2], options.partialrecursive)
                    ):
                        # site match
        ...
        unmatched_other_ts = [
            st for st in other_theme_sites if st not in matched_other_ts
        ]
        ...
        other_only_ts = [
            ots for ots in unmatched_other_ts if ots not in theme_matched_other_ts
        ]
...
class Modification:
    ...
    def resolve_idrefs(self, annotation_by_id):
        assert self.eid in annotation_by_id, (
            "ERROR: undefined ID %s referenced in %s" % (self.eid, self.id)
        )
        self.event = annotation_by_id[self.eid]
        assert self.event.type in event_types, (
            "ERROR: non-Event %s referenced in %s" % (self.eid, self.id)
        )
...
def parse_textbound_line(l, fn):
    ...
    if len(fields) == 3:
        ...
        assert False, (
            "FORMAT ERROR: unexpected number (%d) of tab-separated fields: line '%s' in %s"
            % (len(fields), l, fn)
        )
    ...
    if not re.match(r"^[GT]\d+$", id):
        ...
        pass
    assert re.match(r"^[A-Z]\d+$", id), (
        "FORMAT ERROR: ID not in format '[GT][0-9]+': line '%s' in %s" % (l, fn)
    )
    ...
    assert len(fields) == 3, (
        "FORMAT ERROR: unexpected number of space-separated fields: line '%s' in %s"
        % (l, fn)
    )
    type, start, end = fields
    assert type in textbound_types, (
        "FORMAT ERROR: disallowed type for text-bound annotation: line '%s' in %s"
        % (l, fn)
    )

    # start and end are offsets into the text
    assert re.match(r"^\d+$", start) and re.match(r"^\d+$", end), (
        "FORMAT ERROR: offsets not in '[0-9]+' format: line '%s' in %s" % (l, fn)
    )
    start, end = int(start), int(end)
    assert start < end, (
        "FORMAT ERROR: start offset not smaller than end offset: line '%s' in %s"
        % (l, fn)
    )
    return Textbound(id, type, start, end, text)


# parses and verifies the given text line as a given entity (Protein, Entity)
def parse_given_entity_line(l, fn, document_text):
    ...
    assert e.type in given_types, (
        "FORMAT ERROR: non-given type not allowed here: line '%s' in %s" % (l, fn)
    )
...
def parse_event_line(l, fn):
    ...
    l = re.sub(r"\s*\#.*", "", l)  # コメントを無視

    # タブで分割
    fields = l.split("\t")
    assert len(fields) == 2, (
        "FORMAT ERROR: unexpected number of tab-separated fields: line '%s' in %s"
        % (l, fn)
    )
    id, annotation = fields

    # IDは"R[0-9]+"の形式
    assert re.match(r"^E\d+$", id), (
        "FORMAT ERROR: ID not in format 'E[0-9]+': line '%s' in %s" % (l, fn)
    )

    # イベントタイプと引数を解析
    fields = re.split(" +", annotation)
    # assert len(fields) >= 2, "FORMAT ERROR: unexpected number of space-separated fields: line here '%s' in %s" % (l, fn)
    type_tid, args = fields[0], fields[1:]

    parts = type_tid.split(":")
    assert len(parts) == 2, (
        "FORMAT ERROR: event type not in 'TYPE:ID' format: line '%s' in %s" % (l, fn)
    )
    type, tid = parts
    assert type in event_types, (
        "FORMAT ERROR: disallowed type for event annotation: line '%s' in %s" % (l, fn)
    )
    ...
    for a in args:
        ...
        parts = a.split(":")
        assert len(parts) == 2, (
            "FORMAT ERROR: argument type not in 'ARG:ID' format: line '%s' in %s"
            % (l, fn)
        )
        arg, aid = parts
        assert valid_argument_name(arg), (
            "FORMAT ERROR: invalid argument name: line '%s' in %s" % (l, fn)
        )
        assert re.match(r"^[TE]\d+", aid), (
            "FORMAT ERROR: invalid id in argument: line '%s' in %s" % (l, fn)
        )
...
def parse_modification_line(l, fn):
    ...
    if not options.spliteventeval:
        assert re.match(r"^M\d+$", id), (
            "FORMAT ERROR: ID not in format 'M[0-9]+': line '%s' in %s" % (l, fn)
        )
    ...
    type, eid = fields
    assert type in modification_types, (
        "FORMAT ERROR: disallowed type for modification annotation: line '%s' in %s"
        % (l, fn)
    )
...
def parse_equiv_line(l, fn):
    ...
    assert type == "Equiv", (
        "FORMAT ERROR: non-'Equiv' type for '*' id: line '%s' in %s" % (l, fn)
    )

    # IDs are in the "T[0-9]+" format.
    for id in ids:
        assert re.match(r"^[A-Z]\d+$", id), (
            "FORMAT ERROR: IDs not in format 'T[0-9]+': line '%s' in %s" % (l, fn)
        )
    ...
    for id in ids:
        if id in seen_ids:
            print(
                sys.stderr,
                "Note: id '%s' appears multiple times in Equiv '%s', ignoring repetition"
                % (id, l),
            )
        else:
            seen_ids.add(id)
...
def parse_event_file(a2file, document_text, annotation_by_id):
    ...
    for l in a2file:
        ...
        assert len(l) > 0 and l[0] in (
            ...
            "A",
            "N",
        ), "FORMAT ERROR: line doesn't start with valid ID: line '%s' in %s" % (
            l,
            a2file.name,
        )
        ...
        if l[0] == "*":
            ...
            # print(sys.stderr, "Note: ignoring non-event annotation line '%s' in %s" % (l, a2file.name))
...
def remove_idref_failing_events(annotation_by_id, fn):
    while True:
        dropped_event = False
        # 辞書のキーをリストとしてコピーしてから反復処理を行う
        annids = list(annotation_by_id.keys())
        for i in annids:
            a = annotation_by_id[i]
            try:
                a.resolve_idrefs(annotation_by_id)
            except AssertionError as e:
                print(f"Dropping event {i} in {fn}: {e!s}")
                del annotation_by_id[i]
                dropped_event = True
        if not dropped_event:
            return
...
def a2file_PMID(fn):
    ...
    # m = re.match(r'^(PMID|PMC)?\d+.*\.a2$', bfn)
    # print(f"Processing file: {fn}")
    if not m:
        usage_exit("error: filename '%s' not in expected format" % bfn)

    PMID = m.group(1)  # ここでPMIDを定義する
    # print(f"Extracted PMID: {PMID}")
    return PMID


# parse and verify reference files
def parse_reference_files(PMID, t2_tasksuffix=""):
    ...
    # read other annotations (".a2 file")
    a2_fn = os.path.join(options.refdir, PMID + ".a2" + t2_tasksuffix)
    a2_f = open_annotation_file(a2_fn)
    annotation_by_id, equiv_annotations = parse_event_file(
        a2_f, document_text, annotation_by_id
    )
    a2_f.close()
...
args = [
    options.preddir + "/" + file_name
    for file_name in os.listdir(options.preddir)
    if os.path.isfile(os.path.join(options.preddir, file_name))
    and file_name.endswith(".a2")
]
# args = [file_name for file_name in os.listdir(options.preddir) if
...
if options.spliteventeval:
    ...
    known_suffixes |= set(
        [s + "a" for s in known_suffixes] + [s + "s" for s in known_suffixes]
    )
...
if len(lastsuffixes) > 1:
    usage_exit(
        "error: given files don't all share the task suffix (%s given)"
        % ",".join(lastsuffixes)
    )
...
for PMID in PMIDs:
    ...
    try:
        document_text, gold_ann_by_id, gold_equiv = parse_reference_files(
            PMID, reference_t2_tasksuffix
        )
        # print("pmid", PMID)
    except AssertionError as e:
        ...
        print(
            sys.stderr,
            "ERROR: Failed to read gold annotation for PMID (skipping)",
            PMID,
            ":",
            e,
        )
...
for fn in args:
    ...
    try:
        ...
        for a in [
            e
            for e in ann_by_id.values()
            if e.type == "Negation" or e.type == "Speculation"
        ]:
...
tmp_softboundary, tmp_partialrecursive, tmp_singlepenalty = (
    options.softboundary,
    options.partialrecursive,
    options.singlepenalty,
)
options.softboundary, options.partialrecursive, options.singlepenalty = (
    False,
    False,
    False,
)
...
if options.softboundary:
    for PMID in annotation:
        ...
        gold_textbound = [
            t for t in gold_ann_by_id.values() if isinstance(t, Textbound)
        ]
...
total_match_answer, total_match_gold, total_false_positive, total_false_negative = (
    0,
    0,
    0,
    0,
...
(
    total_type_match_answer,
    total_type_match_gold,
    total_type_false_positive,
    total_type_false_negative,
) = {}, {}, {}, {}
total_type_partial_match, total_type_over_match = {}, {}
for t in event_types | modification_types:
    total_type_match_answer[t] = total_type_match_gold[t] = total_type_false_positive[
        t
...
for PMID in PMIDs:
    ...
    if PMID not in gold_annotation or PMID not in annotation:
        ...
        print(
            sys.stderr,
            "# NOTE: SKIPPING %s, READ FAILED: RESULTS WILL NOT BE VALID FOR THE FULL DATASET!"
            % fn,
        )
        ...
        for ge in gold_events:
            if not options.singlepenalty:
                ...
                if part and over:
                    ...
                    assert e.matches(ge, match_partial=False), (
                        "INTERNAL ERROR: two partial matches but no full match"
                    )
                    ge.mark_matching(e)
                    e.mark_matching(ge)
                elif part:
    ...
    for t in event_types | modification_types:
        type_match_answer[t] = type_match_gold[t] = type_false_positive[
            t
        ] = type_false_negative[t] = 0
            ...
            if e.has_matched():
                ...
                print(
                    ...
                    % (
                        e.type,
                        PMID + "-" + e.id,
                        ",".join([PMID + "-" + x.id for x in e.matching]),
                    ),
                )
            elif e.has_partially_matched():
                ...
                print(
                    ...
                    % (
                        e.type,
                        PMID + "-" + e.id,
                        ",".join([PMID + "-" + x.id for x in e.partially_matched]),
                    ),
                )
            else:
            ...
            if ge.has_partially_matched():
                ...
                print(
                    ...
                    % (
                        ge.type,
                        PMID + "-" + ge.id,
                        ",".join([PMID + "-" + x.id for x in ge.partially_matched]),
                    ),
                )
    ...
    # sanity
    assert match_gold + over_match + false_negative == len(gold_events), (
        "INTERNAL ERROR"
    )
    assert match_answer + partial_match + false_positive == len(events), (
        "INTERNAL ERROR"
    )
...
if options.outputdir is not None:
    ...
    for PMID in PMIDs:
        ...
        submission_textbound = [
            a for a in ann_by_id.values() if isinstance(a, Textbound)
        ]
        gold_textbound = [
            a for a in gold_ann_by_id.values() if isinstance(a, Textbound)
        ]
        ...
        for aid, ann in ann_by_id.items():
            ...
            if isinstance(ann, Textbound):
                ...
                for arg, argid in ann.args:
                    ...
                    if (
                        argid in gold_ann_by_id
                        and gold_ann_by_id[argid].type in given_types
                    ):
                        ...
                        argid in ann_by_id
                        and ann_by_id[argid].type in event_types
                        and ann_by_id[argid].has_matched()
                    ):
        ...
        try:
            ...
            print(
                sys.stderr,
                "Error: failed to write %s, %s, %s: %s"
                % (outtxtfn, outa1fn, outa2fn, e),
            )
        ...
        for aid, ann in gold_ann_by_id.items():
            ...
            if isinstance(ann, Event) and not isinstance(ann, ModificationEvent):
                if not ann.has_matched():
                    print(
                        outa2,
                        "#%d\tFalse_negative %s\tFalse negative (in reference, not in submission)"
                    ...
                    print(
                        ...
                        "#%d\tTrue_positive %s\t%s"
                        % (
                            next_free_comment_idx,
                            ann.id,
                            "Matches " + ",".join([e.id for e in ann.matching]),
                        ),
                    )
                next_free_comment_idx += 1
        ...
        for aid, ann in ann_by_id.items():
            ...
            if isinstance(ann, Event) and not isinstance(ann, ModificationEvent):
                ...
                if not ann.has_matched():
                    ...
                    print(
                        outa2,
                        "#%d\tFalse_positive %s\tFalse positive (in submission, not in reference)"
                        % (next_free_comment_idx, ann.id),
                    )
                    next_free_comment_idx += 1
    ...
    try:
        ...
        print(
            statsout,
            '"DOCUMENT"\t"TP(gold)"\t"TP(answer)"\t"FP"\t"FN"\t"recall"\t"precision"\t"F-score"',
        )
...
if (
    sum(
        [
            total_type_match_gold[t]
            + total_type_over_match[t]
            + total_type_false_negative[t]
            for t in ("Negation", "Speculation")
        ]
    )
    != 0
):
...
for t in event_types:
    if (
        t not in output_event_type_order
        and total_type_match_answer[t]
        + total_type_match_gold[t]
        + total_type_over_match[t]
        + total_type_partial_match[t]
        + total_type_false_positive[t]
        + total_type_false_negative[t]
        != 0
    ):
        # cram in here
...
if number_of_documents_evaluated != len(args):
    print(
        "NOTE: evaluation succeeded only for %d/%d documents. Results will not be valid for the whole dataset."
...
if not options.singlepenalty:
    ...
    print(
        "      Event Class         gold (match/part)   answer (match/part)   recall    prec.   fscore"
    )
print(divider)
...
def print_subset_total(eventset, totalstr):
    ...
    precision, recall, F = prec_rec_F(
        subtotal_match_answer,
        subtotal_match_gold + subtotal_over_match,
        subtotal_false_positive,
        subtotal_false_negative,
    )
    gold_event_count = (
        subtotal_match_gold + subtotal_over_match + subtotal_false_negative
    )
    answer_event_count = (
        subtotal_match_answer + subtotal_partial_match + subtotal_false_positive
    )

    if not options.singlepenalty:
        print(
            "   %s    %5d ( %4d)     %4d ( %4d)   %6.2f   %6.2f   %6.2f"
            % (
                totalstr,
                gold_event_count,
                subtotal_match_gold,
        ...
        print(
            "   %s    %5d ( %4d/%4d)     %4d ( %4d/%4d)   %6.2f   %6.2f   %6.2f"
            % (
                totalstr,
                gold_event_count,
                subtotal_match_gold,
    ...
    if "[TOTAL]" in totalstr:
        ...
        assert (
            subtotal_match_gold == total_match_gold
            and subtotal_match_answer == total_match_answer
            and subtotal_over_match == total_over_match
            and subtotal_partial_match == total_partial_match
        ), "Error: totals do not agree!"
...
for t in output_event_type_order:
    if "TOTAL" in t:
        ...
        if not options.singlepenalty:
            print(
                " %s    %4d ( %4d)    %5d ( %4d)   %6.2f   %6.2f   %6.2f"
                % (
                    t[:MAX_TYPE_PRINT_LEN].center(MAX_TYPE_PRINT_LEN),
                    gold_event_count,
                    total_type_match_gold[t],
                    answer_event_count,
                    total_type_match_answer[t],
            ...
            print(
                " %s    %4d ( %4d/%4d)    %5d ( %4d/%4d)   %6.2f   %6.2f   %6.2f"
                % (
                    t[:MAX_TYPE_PRINT_LEN].center(MAX_TYPE_PRINT_LEN),
                    gold_event_count,
                    total_type_match_gold[t],
